...
/// Reinterpret a raw integer as a fieldless C-like enum without checking the
/// discriminant. Modern Rust panics on `mem::transmute` into an enum when the
/// value does not match any variant (e.g. OR-ed bitflags), so we write the bits
/// through a `MaybeUninit` instead.
#[inline(always)]
pub unsafe fn bits_to<E, I: Copy>(value: I) -> E {
    let mut out = std::mem::MaybeUninit::<E>::uninit();
    std::ptr::write(out.as_mut_ptr() as *mut I, value);
    out.assume_init()
}
...
impl SCITER_CREATE_WINDOW_FLAGS {
  ...
  pub const SW_TITLEBAR: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 1);
  /// has resizeable frame.
  pub const SW_RESIZEABLE: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 2);
  /// is tool window.
  pub const SW_TOOL: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 3);
  /// has minimize / maximize buttons.
  pub const SW_CONTROLS: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 4);
  /// glassy window - "Acrylic" on Windows and "Vibrant" on macOS.
  pub const SW_GLASSY: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 5);
  /// transparent window (e.g. `WS_EX_LAYERED` on Windows, macOS is supported too).
  pub const SW_ALPHA: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 6);
  /// main window of the app, will terminate the app on close.
  pub const SW_MAIN: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 7);
  /// the window is created as topmost window.
  pub const SW_POPUP: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 8);
  /// make this window inspector ready.
  pub const SW_ENABLE_DEBUG: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 9);
  /// it has its own script VM.
  pub const SW_OWNS_VM: SCITER_CREATE_WINDOW_FLAGS = SCITER_CREATE_WINDOW_FLAGS(1 << 10);
}
...
impl ::std::ops::BitOr for SCITER_CREATE_WINDOW_FLAGS {
  ...
  fn bitor(self, rhs: Self::Output) -> Self::Output {
    SCITER_CREATE_WINDOW_FLAGS(self.0 | rhs.0)
  }
}
...
impl ::std::ops::Not for SCITER_CREATE_WINDOW_FLAGS {
  type Output = SCITER_CREATE_WINDOW_FLAGS;
  fn not(self) -> Self::Output {
    SCITER_CREATE_WINDOW_FLAGS(!self.0)
  }
}

impl From<SCITER_CREATE_WINDOW_FLAGS> for UINT {
  fn from(v: SCITER_CREATE_WINDOW_FLAGS) -> UINT {
    v.0
  }
}
