...
fn is_detach_event(evtg: UINT, params: LPVOID) -> bool {
	let evtg : EVENT_GROUPS = unsafe { ::capi::scdef::bits_to(evtg) };
...
fn process_events(me: &mut dyn EventHandler, he: HELEMENT, evtg: UINT, params: LPVOID) -> BOOL
{
	...
	let result = match evtg {
      ...
      use dom::event::EventReason;
			let code :BEHAVIOR_EVENTS = unsafe{ ::capi::scdef::bits_to(nm.cmd & 0x0_0FFF) };
			let phase: PHASE_MASK = unsafe { ::capi::scdef::bits_to(nm.cmd & 0xFFFF_F000) };
			let reason = match code {
				BEHAVIOR_EVENTS::EDIT_VALUE_CHANGED | BEHAVIOR_EVENTS::EDIT_VALUE_CHANGING => {
					let reason: EDIT_CHANGED_REASON = unsafe{ ::capi::scdef::bits_to(nm.reason as UINT) };
				...
				_ => {
					let reason: CLICK_REASON = unsafe{ ::capi::scdef::bits_to(nm.reason as UINT) };
    ...
    EVENT_GROUPS::HANDLE_METHOD_CALL => {
      ...
      let nm = unsafe { & *scnm };
      let code: BEHAVIOR_METHOD_IDENTIFIERS = unsafe { ::capi::scdef::bits_to((*nm).method) };
