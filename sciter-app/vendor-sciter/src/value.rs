...
impl Value {
  ...
  #[allow(clippy::mut_from_ref)]
	fn ensure_tmp_mut(&self) -> &mut Value {
		let me = self.ensure_tmp_ptr();
		return unsafe { &mut *me };
	}

	fn ensure_tmp_ptr(&self) -> *mut Value {
		let tmp = self.tmp.get();
		let existing = unsafe { *tmp };
		if existing.is_null() {
			let boxed = Box::new(Value::new());
...
impl Drop for Value {
	fn drop(&mut self) {
		...
		if !unsafe { *self.tmp.get() }.is_null() {
			let _drop_tmp = unsafe { Box::from_raw(*self.tmp.get()) };
		}
